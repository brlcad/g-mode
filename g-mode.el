...
(defun g-mode--append-object-copy (obj new-name bin-buf)
  "Build a copy of OBJ named NEW-NAME and append it to BIN-BUF.
Return the byte position where the new record was appended.
The new record preserves OBJ's type, flags, and interior data."
  (let* ((hflags (cdr (assq 'hflags obj)))
    ...
    (let* ((olen-str (g-mode--uint-to-bytes olen-chunks new-olen-bytes))
           ...
           (appended-pos (with-current-buffer bin-buf (point-max))))
      (with-current-buffer bin-buf
        (save-excursion
              ...
              (unless (string= old-name new-name)
                ...
                (let* ((old-nlen (1+ (length old-name)))
                  ...
                  (if (<= new-nlen old-nlen)
                    ...
                    (let ((appended-pos (g-mode--append-object-copy obj new-name bin-buf)))
